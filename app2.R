...
setwd("~/Documents/SEA/shiny/Saudi-Arabia-Shiny")
library(readxl)
library(sf)
library(ggplot2)
...
leaflet() %>%
  ...
  addLegend("bottomright", pal = pal, values = values(clipped.raster.dpol), title = htmltools::HTML("PM<sub>2.5</sub>")) %>%
  addScaleBar(position = c("bottomleft"))
...
ui <- fluidPage(
  tags$head(
    tags$style(HTML("
      ...
      .map-container {
        margin-bottom: 20px; /* Adds space between maps */
      }
    "))
  ),
  titlePanel(
...
server <- function(input, output, session) {
  ...
  observeEvent(input$filedata, {
    ...
    output$mapOutput1 <- renderUI({
      div(leafletOutput("mymap1", height = "300px"), class = "map-container")
    })
    output$mapOutput2 <- renderUI({
      div(leafletOutput("mymap2", height = "300px"), class = "map-container")
    })
  })
  ...
  output$report <- renderText({
    ...
    paste("The highest relative risk (RR) in year", year, "is in", highest_risk_region, "with a value of", highest_risk_value)
  })
